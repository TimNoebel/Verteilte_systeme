...
int sockeds_array[50];
int glob_std_sockfd = 0;
int glob_is_default_sockfd_initialized;
...
void* threadServerFunction(void *arg) {
	int sockfd = *(int*)arg;
	char buffer[MESG_SIZE];

	printf("thread Nr. %d reports for Duty\n", number_connections);
...
int setupConnection(int _port) {
	...
	if(!glob_is_default_sockfd_initialized) {
		...
		glob_std_sockfd = sockfd;
		glob_is_default_sockfd_initialized = 1;
	} else {
		sockfd = glob_std_sockfd;
	}
...
int firstTouch(int _port) {
	...
	printf("received: %s\n", buffer);
	((int*)buffer)[0] = _port + number_connections;
	write(sockfd, buffer, MESG_SIZE);
	close(sockfd);
	return _port + number_connections;
}

int permantConnection(int _port) {
	printf("setup permanent connection on port: %i\n", _port);
	int sockfd = setupConnection(_port);
	return sockfd;
}
...
int main(int argc, char** argv) {
	...
	while(number_connections <= 50) {
		port = firstTouch(STD_PORT);
		sockeds_array[number_connections] = permantConnection(port);

		pthread_create(&se, NULL, threadServerFunction, &sockeds_array[number_connections]);
	}
	pthread_join(se, NULL);

	//serverFunction(sockeds_array[number_connections]);

	for(int i = number_connections; i > 0; i--) {
		printf("closing socket: %i\n", sockeds_array[i]);
		close(sockeds_array[i]);
	}
